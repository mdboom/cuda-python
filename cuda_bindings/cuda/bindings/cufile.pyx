...
cdef class StatsLevel1:
    ...
    def readv_ops(self, val):
        if self._readonly:
            raise ValueError("This StatsLevel1 instance is read-only")
        cdef OpCounter val_ = val
        memcpy(<void *>&(self._ptr[0].readv_ops), <void *>(val_._get_ptr()), sizeof(CUfileOpCounter_t) * 1)
    ...
    def writev_ops(self, val):
        if self._readonly:
            raise ValueError("This StatsLevel1 instance is read-only")
        cdef OpCounter val_ = val
        memcpy(<void *>&(self._ptr[0].writev_ops), <void *>(val_._get_ptr()), sizeof(CUfileOpCounter_t) * 1)
